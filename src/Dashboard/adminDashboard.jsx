...
function ProfilePage() {
    ...
    useEffect(() => {
        if (userProfile) {
            ...
            setFormData({
                ...userProfile,
                email: userProfile.email || "", // Assuming email is part of the User class
            });
        } else {
                    ...
                    <Grid item xs={12} md={8} lg={9}>
                        <Box>
                            ...
                            <TextField
                                ...
                                disabled
                            />
                            {/* Password field (not editable directly here) */}
                            <TextField
                                label="كلمة المرور"
                                fullWidth
                                margin="normal"
...
function PaidAdvertismentPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = RealEstateDeveloperAdvertisement.subscribeActiveAds(
            (ads) => {
                dispatch(setDeveloperAds(ads.map(ad => ({ ...ad, id: ad.id }))));
            },
            (err) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = FinancingAdvertisement.subscribeActiveAds(
            (ads) => {
                dispatch(setFunderAds(ads.map(ad => ({ ...ad, id: ad.id }))));
            },
            (err) => {
...
function ClientAdvertismentPage() {
    ...
    const loading = useSelector((state) => state.advertisements.loading); // 'idle', 'pending', 'succeeded', 'failed'
    ...
    useEffect(() => {
        // Dispatch the async thunk to fetch advertisements when the component mounts
        dispatch(fetchAdvertisements());

        // For real-time updates in an admin panel, you would typically use onSnapshot.
        // If you want real-time updates with Redux, you'd integrate onSnapshot
        // directly here and dispatch a custom action to update the `list` when changes occur.
        // For simplicity and focusing on asyncThunks, we're doing a one-time fetch.
        // To re-fetch on updates (like after delete), you'd dispatch fetchAdvertisements() again.
    }, [dispatch]);
    ...
    const handleConfirmDelete = async () => {
        if (adToDelete) {
            try {
                ...
                await dispatch(deleteAdvertisement(adToDelete.id)).unwrap(); // .unwrap() to catch errors
                setSnackbar({ open: true, message: "الإعلان تم حذفه بنجاح!", severity: "success" });
            } catch (err) {
    ...
    const handleToggleAdStatus = async (ad) => {
        try {
            let newStatus;
            if (ad.ad_status === 'تحت العرض') {
                newStatus = 'منتهي';
            } else if (ad.ad_status === 'منتهي') {
                newStatus = 'تحت العرض';
            } else if (ad.ad_status === 'تحت التفاوض') {
                newStatus = 'تحت العرض'; // Assuming 'تحت التفاوض' transitions to 'تحت العرض'
            } else {
                newStatus = 'تحت العرض'; // Default or handle other statuses as needed
            }
    ...
    const handleEditClick = (ad) => {
        // TODO: Implement navigation to an edit page or open a modal for editing
        console.log("Edit advertisement:", ad);
        setSnackbar({ open: true, message: `تحرير الإعلان: ${ad.title}`, severity: "info" });
        // Example: navigate(`/admin/client-advertisements/edit/${ad.id}`, { state: { adData: ad } });
    };
    ...
    const handleSnackbarClose = (event, reason) => {
        ...
        setSnackbar({ ...snackbar, open: false });
        dispatch(clearAdvertisementsError()); // Clear Redux error when Snackbar closes
    };
    // --- End Operations ---
        ...
        { field: 'date_of_building', headerName: 'تاريخ الإنشاء', width: 150, editable: false },
        {
            ...
            renderCell: (params) => (
                <Box sx={{ display: 'flex', gap: 0.5 }}>
                    ...
                    <Tooltip
                        title={
                            params.row.ad_status === 'تحت العرض'
                                ? "إنهاء العرض"
                                : params.row.ad_status === 'منتهي'
                                    ? "إعادة العرض"
                                    : "تغيير الحالة" // Tooltip for 'تحت التفاوض'
                        }
                    >
                        <IconButton
                            aria-label="toggle status"
                            size="small"
                            onClick={() => handleToggleAdStatus(params.row)}
                            color={
                                params.row.ad_status === 'تحت العرض'
                                    ? 'warning'
                                    : params.row.ad_status === 'منتهي'
                                        ? 'success'
                                        : 'info' // Color for 'تحت التفاوض'
                            }
                            disabled={loading === 'pending'}
                        >
                            {params.row.ad_status === 'تحت العرض' ? (
                                ...
                                <AutorenewIcon fontSize="small" /> // Icon for 'تحت التفاوض'
                            )}
                        </IconButton>
                    </Tooltip>
