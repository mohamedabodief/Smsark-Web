...
// org profile
import { fetchUserProfile, updateUserProfile } from "../../LoginAndRegister/featuresLR/userSlice";
...
function ProfilePage() {
    ...
    const handleSave = async () => {
        ...
        const updates = {
            ...
            email: formData.email, // Include email if it's editable or just for display
        };

        try {
            console.log("OrganizationProfilePage: Dispatching updateUserProfile for UID:", actualUid, "Updates:", updates);
    ...
    const UploadAvatars = () => {
        const handleImageChange = (event) => {
            ...
            if (file) {
                ...
                reader.onloadend = () => {
                    ...
                    // You might also want to save this image URL to Firestore here
                    // by dispatching another thunk to update the user's 'image' field.
                };
        ...
        const handleRemoveImage = () => {
            dispatch(setProfilePic('./admin.jpg')); // Or a default placeholder image
            // Also, dispatch an action to remove the image URL from Firestore
        };
        ...
        return (
            <Box sx={{ p: 2, borderRadius: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
                    ملفات JPG أو PNG بحجم أقصى 5 ميجابايت
                </Typography>
            </Box>
        );
    };
    ...
    // Ensure userProfile.type_of_user is 'organization' before rendering organization fields
    // This is a final safeguard, though ideally, this component is only rendered for organizations.
    if (userProfile.type_of_user !== "organization") {
        return (
            <Box sx={{ p: 3 }}>
                <Alert severity="error">Access Denied: This profile page is for organizations only.</Alert>
            </Box>
        );
    }
