...
class ClientAdvertisement {
    ...
    async adsActivation(days) {
        ...
        setTimeout(() => {
            ClientAdvertisement.getById(this.#id)
                .then(adData => {
                    if (adData && adData.ads && adData.adExpiryTime && adData.adExpiryTime <= Date.now()) {
                        const tempInstance = new ClientAdvertisement(adData);
                        tempInstance.removeAds().catch(e => console.error(e));
                    }
                })
    ...
    static async getById(id) {
        ...
        const snapshot = await getDoc(docRef);
        if (snapshot.exists()) {
            // Return plain object including the Firestore document ID
            return await ClientAdvertisement.#handleExpiry({ id: snapshot.id, ...snapshot.data() });
        }
        return null;
    }
    ...
    static subscribeActiveAds(callback, errorCallback) {
        ...
        return onSnapshot(q, async (querySnapshot) => {
            ...
            for (const docSnap of querySnapshot.docs) {
                ...
                const processedAd = await ClientAdvertisement.#handleExpiry({ id: docSnap.id, ...adData });
    ...
    static async getAll() {
        ...
        for (const docSnap of snapshot.docs) {
            const adData = docSnap.data();
            const ad = await ClientAdvertisement.#handleExpiry({ id: docSnap.id, ...adData });
            if (ad) ads.push(ad);
        }
        return ads;
    }
