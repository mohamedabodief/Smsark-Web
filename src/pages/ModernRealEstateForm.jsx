...
const ModernRealEstateForm = () => {
  ...
  useEffect(() => {
    ...
    if (isEditMode && editData && editData.receipt_image) {
      setReceiptImage(editData.receipt_image);
  ...
  const handleImageUpload = (event) => {
    ...
    if (invalidFiles.length > 0) {
      setImageError("يرجى رفع صور بصيغة JPEG أو PNG وبحجم أقل من 5 ميجابايت");
      return;
    }
  ...
  const onSubmit = async (data) => {
    ...
    // تحقق من وجود صور في وضع التعديل
    if (isEditMode && existingImages.length === 0 && images.length === 0) {
      setImageError("يجب وجود صورة واحدة على الأقل");
      setLoading(false);
      return;
    }
