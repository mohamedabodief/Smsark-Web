...
// Async thunk to fetch financing requests for a specific user
export const fetchFinancialRequestsByUser = createAsyncThunk(
  "financialRequests/fetchByUser",
  async (userId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || "فشل تحميل طلبات التمويل للمستخدم");
    }
  }
);
...
const financialRequestSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchFinancialRequestsByUser.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchFinancialRequestsByUser.fulfilled, (state, action) => {
        state.loading = false;
        state.list = action.payload;
      })
      .addCase(fetchFinancialRequestsByUser.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      });
  },
